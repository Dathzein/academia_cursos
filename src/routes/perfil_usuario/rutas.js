...
router.post('/', asignarPerfil);
router.put('/', eliminarUsuario);

async function consultar(req, res, next) {
    try {
        const items = await controller.obtenerUsuariosPerfil()
...
async function asignarPerfil(req, res, next) {
    try {
        const item = await controller.asignarPerfil(req.body)
        if(!req.body.id){
